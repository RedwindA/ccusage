...
pub fn load_entries(shared: &SharedArgs, report_kind: AgentReportKind) -> Result<Vec<LoadedEntry>> {
    load_entries_with_pricing(shared, report_kind, None)
}
...
pub fn load_entries_with_pricing(
    ...
    crate::progress::track_usage_load(
        ...
        shared.json,
        || load_entries_inner(shared, allow_aggregate_fallback, pricing),
    )
}
...
fn load_entries_inner(
    ...
    for path in paths()? {
        let directory_entries =
            load_entries_from_directory_parts(&path, shared, allow_aggregate_fallback, pricing)?;
...
pub fn load_entries_from_directory(
    ...
    load_entries_from_directory_with_pricing(opencode_dir, shared, false, None)
}
...
fn load_entries_from_directory_with_pricing(
    ...
    let directory_entries =
        load_entries_from_directory_parts(opencode_dir, shared, allow_aggregate_fallback, pricing)?;
...
fn load_entries_from_directory_for_report(
    ...
    load_entries_from_directory_with_pricing(
        opencode_dir,
        shared,
...
mod tests {
    ...
    fn loader_uses_provided_pricing() {
        ...
        let entries =
            load_entries_with_pricing(&shared, AgentReportKind::Daily, Some(&pricing)).unwrap();
