...
impl PricingMap {
    ...
    fn find_unknown_model_prefix(&self, model: &str) -> Option<Pricing> {
        ...
        let mut best = longest_model_prefix_match(&self.entries, &self.exact_only, model);
        if self.enable_models_dev_fallback
            && let Some(pricing) = models_dev_pricing()
        {
            best = prefer_longer_prefix(
                best,
                longest_model_prefix_match(&pricing.entries, &pricing.exact_only, model),
            );
        }
        if self.enable_embedded_models_dev_fallback {
            best = prefer_longer_prefix(
                best,
                longest_model_prefix_match(
                    &embedded_models_dev_pricing().entries,
                    &embedded_models_dev_pricing().exact_only,
                    model,
                ),
            );
        }
    ...
    fn context_limit_unknown_model_prefix(&self, model: &str) -> Option<u64> {
        ...
        let mut best = longest_model_prefix_match(&self.context_limits, &self.exact_only, model);
        if self.enable_models_dev_fallback
            && let Some(pricing) = models_dev_pricing()
        {
            best = prefer_longer_prefix(
                best,
                longest_model_prefix_match(&pricing.context_limits, &pricing.exact_only, model),
            );
        }
        if self.enable_embedded_models_dev_fallback {
            best = prefer_longer_prefix(
                best,
                longest_model_prefix_match(
                    &embedded_models_dev_pricing().context_limits,
                    &embedded_models_dev_pricing().exact_only,
                    model,
                ),
            );
...
fn longest_model_prefix_match<'a, V: Copy>(
    ...
    entries
        .iter()
        .filter(|(candidate, _)| !exact_only.contains(candidate.as_str()))
        .filter(|(candidate, _)| {
...
mod tests {
    ...
    fn fuzzy_match_requires_model_key_boundaries() {
        ...
        assert!(
            pricing
                .find_entry("claude-opus-4.70", Fuzzy::Allowed)
                .is_none()
        );
    }
    ...
    fn fuzzy_match_does_not_fall_back_across_numeric_model_versions() {
        ...
        assert!(
            pricing
                .find_entry("claude-opus-4.8-20260528", Fuzzy::Allowed)
                .is_none()
        );
        assert!(
            pricing
                .find_entry("claude-opus-4-9", Fuzzy::Allowed)
                .is_none()
        );
        assert!(
            pricing
                .find_entry("claude-opus-5", Fuzzy::Allowed)
                .is_none()
        );
        assert!(
            pricing
        ...
        assert!(
            pricing
                .find_entry("claude-opus-4-20250514", Fuzzy::Allowed)
                .is_some()
        );
    }
