    ...
    for row in rows {
        ...
        if let Some(agent_breakdowns) = row.agent_breakdowns.as_ref() {
            for breakdown in agent_breakdowns {
                ...
                if include_sources {
                    push_source_breakdown_rows(&mut table, breakdown, compact, show_users, shared);
                }
            ...
            if include_sources {
                push_source_breakdown_rows(&mut table, row, compact, show_users, shared);
            }
